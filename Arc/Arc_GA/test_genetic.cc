...
int main(){
    ...
    double*  adj = new double[adj_size*adj_size];
    for(int i=0; i < adj_size*adj_size; i++){
        adj[i] = adj_[i];
    }

    double*  upper_bounds = new double[n_tolls];
    for(int i=0; i < n_tolls; i++){
        upper_bounds[i] = upper_bounds_[i];
    }
