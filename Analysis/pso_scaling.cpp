...
int main() {
    ...
    for (int i=0;i<files.size();++i) {
        ...
        outfile <<"loc_"<<n_loc<<"_comm_"<<file_reader.n_commodities<<"_toll_"<<n_highway<<": time init = "<<((float)end1 - (float)start1)/CLOCKS_PER_SEC<<"s time run = "<<((float)end2 - (float)start2)/CLOCKS_PER_SEC<<"  obj = "<<s.get_best_val()<<std::endl;
    }
