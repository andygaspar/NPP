...
class GeneticPsoCpp:
    def __init__(self, upper_bounds, lower_bounds, commodities_tax_free: np.array, n_users: np.array, transfer_costs: np.array,
        ...
        self.lib = ctypes.CDLL('Path/CPP/libs/gapso_bridge.so')

        self.lib.GeneticPso_.argtypes = [ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
        ...
        self.lib.GeneticPso_.restype = ctypes.c_void_p
        ...
        self.genetic = self.lib.GeneticPso_(upper_bounds.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
